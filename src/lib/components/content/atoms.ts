...
export const getMDXContentAtom = atom(null, async (get, set, filename: string) => {
...
export const prefetchMDXContentAtom = atom(null, async (get, set, filenames: string[]) => {
  const cache = get(mdxContentCacheAtom)
  ...
  await Promise.allSettled(uncachedFiles.map((filename) => set(getMDXContentAtom, filename)))
})

export const clearMDXCacheAtom = atom(null, (_get, set) => {
