...
const MDXCard = ({
  ...
  <Card className={cn("my-3", className)} {...props}>
    ...
    {children && (
      <CardContent className="px-4 pt-3 pb-4 text-sm [&>*:last-child]:mb-0 [&>*:first-child]:mt-0">
        {children}
      </CardContent>
    )}
  </Card>
...
const MDXAlert = ({
  ...
  return (
    ...
    >
      ...
      {children && (
        <AlertDescription className="text-sm text-muted-foreground [&>*:last-child]:mb-0">{children}</AlertDescription>
      )}
    </Alert>
  )
}
...
export const mdxComponents = {
  ...
  h3: (props: React.HTMLAttributes<HTMLHeadingElement>) => (
    <h3 className="text-base font-semibold mb-2 mt-4" {...props} />
  ),
  ...
  pre: (props: React.HTMLAttributes<HTMLPreElement>) => (
    <pre className="my-4 overflow-x-auto p-4 bg-muted text-xs" {...props} />
  ),
